...
# --- BEGIN: Custom logic for handling common/mild symptoms (REFINED) ---
        
        VERY_COMMON_MILD_SYMPTOMS_CANONICAL = [
            'continuous_sneezing', 'runny_nose', 'throat_irritation', 'congestion', 
            'loss_of_smell', 'mild_fever', 'cough', 'headache',
        ]
        ...
        HEURISTIC_SEVERE_DISEASES_KEYWORDS = [
            ...
            'Hepatitis', 'Diabetes', # Covers "Diabetes" and "Diabetes " due to .strip().lower()
            'Typhoid', 'Malaria', 'Dengue', 'Jaundice'
        ]
        # desc_dict should be globally available in app.py
        if "Diabetes " in desc_dict and "Diabetes" not in HEURISTIC_SEVERE_DISEASES_KEYWORDS:
             HEURISTIC_SEVERE_DISEASES_KEYWORDS.append("Diabetes") # Ensure distinct add

        is_prediction_severe = False
        # prediction variable should be available from the line above this block
        if 'prediction' in locals() or 'prediction' in globals():
            for keyword in HEURISTIC_SEVERE_DISEASES_KEYWORDS:
                if keyword.strip().lower() in prediction.strip().lower(): # Ensure both are stripped and lowercased
                    is_prediction_severe = True
                    break
        
        warning_displayed = False 
        
        # user_input is the list of mapped symptoms in app.py context
        if 'user_input' in locals() or 'user_input' in globals():
            if len(user_input) == 1 and user_input[0] in SINGLE_AMBIGUOUS_SYMPTOMS_OF_CONCERN and is_prediction_severe:
                symptom_name = user_input[0].replace('_', ' ')
                custom_message = (
                    f"The single symptom you provided ('**{symptom_name}**') can be associated with a wide range of conditions, from mild to severe. "
                    f"While the system noted a possibility of **{prediction}**, diagnosing based on a single symptom is often unreliable. "
                    f"If you have other symptoms, please include them for a more refined suggestion. "
                    f"If this symptom is persistent, worsens, or you have significant concerns, please consult a doctor promptly."
                )
                st.warning(f"⚠️ **Important Context:** {custom_message}")
                warning_displayed = True
            
            elif not warning_displayed and user_input: # Rule 2: if Rule 1 not triggered and user_input is not empty
                ...
                if is_input_common_mild_only and is_prediction_severe:
                    symptom_list_str = ', '.join(s.replace('_', ' ') for s in user_input)
                    custom_message = (
                        f"The symptoms you provided ({symptom_list_str}) are common and can be associated with various conditions. "
                        ...
                        f"Please monitor your symptoms. If they persist, worsen, or if you have concerns, please consult a doctor."
                    )
                    st.warning(f"⚠️ **Important Note:** {custom_message}")
                    warning_displayed = True
        else:
            # This else implies 'user_input' was not found, which would be an issue with app.py structure.
            # This debug message could be removed for production code if app.py structure is stable.
            print("Debug from warning logic: 'user_input' (mapped symptoms) not found in local/global scope.")
